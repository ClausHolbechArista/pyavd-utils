...
pub struct Str {
    ...
    /// Perl shorthand classes use ASCII semantics, while wildcards and explicit character
    /// classes operate safely on Unicode strings. Unicode properties are not supported.
...
impl Pattern {
    ...
    pub fn get_compiled_pattern(&self) -> Result<&Regex, &Error> {
        self.compiled_pattern
            .get_or_init(|| {
                let translated_pattern = translate_ascii_perl_classes(&self.pattern)?;
                let anchored_pattern = format!("^(?:{translated_pattern})$");
                RegexBuilder::new(&anchored_pattern)
                    // `false` makes `.`, negated classes, and shorthand complements byte-oriented,
                    // which is invalid for Rust UTF-8 strings. The shorthand classes are rewritten
                    // above to retain AVD's ASCII semantics while keeping the rest Unicode-safe.
                    .unicode_mode(true)
                    .build()
            })
            .as_ref()
...
fn translate_ascii_perl_classes(pattern: &str) -> Result<String, Error> {
        ...
        match escaped {
            ...
            'p' | 'P' => {
                return Err(Error::ParseError(
                    position,
                    ParseError::InvalidEscape(format!(r"\{escaped}")),
                ));
            }
            _ => {
                translated.push(character);
...
mod tests {
    ...
    #[test]
    fn perl_classes_use_ascii_semantics() {
    ...
    #[test]
    fn complemented_perl_classes_use_ascii_semantics() {
